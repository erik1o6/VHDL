...
va_list vap)
{
    extern HSim__s6 * createworkMramtest__tbw1(const char*);
    HSim__s6 *blk = createworkMramtest__tbw1(label); 
    return blk;
}
...
{
    extern HSim__s6* createWork_ramtest_struct(const char*);
    HSim__s6 *blk = createWork_ramtest_struct(label);
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","ramtest_tbw1", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("ramtest/struct", (HSimInstFactoryPtr)IF2, true);
        (*cfgvh).addVlogModule("ramtest", (HSimInstFactoryPtr)IF3, true);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMramtest__tbw1(const char*);
        topvl = (HSim__s5*)createworkMramtest__tbw1("ramtest_tbw1");
