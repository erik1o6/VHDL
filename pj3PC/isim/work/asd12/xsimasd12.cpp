...
va_list vap)
{
    extern HSim__s6 * createworkMasd12(const char*);
    HSim__s6 *blk = createworkMasd12(label); 
    return blk;
}
...
{
    extern HSim__s6* createWork_eapc_behavioral(const char*);
    HSim__s6 *blk = createWork_eapc_behavioral(label);
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","asd12", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("eapc/behavioral", (HSimInstFactoryPtr)IF2, true);
        (*cfgvh).addVlogModule("eapc", (HSimInstFactoryPtr)IF3, true);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMasd12(const char*);
        topvl = (HSim__s5*)createworkMasd12("asd12");
