...
va_list vap)
{
    extern HSim__s6 * createworkMloro42(const char*);
    HSim__s6 *blk = createworkMloro42(label); 
    return blk;
}
...
{
    extern HSim__s6* createWork_smicro_struct(const char*);
    HSim__s6 *blk = createWork_smicro_struct(label);
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","loro42", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("smicro/struct", (HSimInstFactoryPtr)IF2, true);
        (*cfgvh).addVlogModule("smicro", (HSimInstFactoryPtr)IF3, true);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMloro42(const char*);
        topvl = (HSim__s5*)createworkMloro42("loro42");
