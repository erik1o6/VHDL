...
va_list vap)
{
    extern HSim__s6 * createworkMlol232(const char*);
    HSim__s6 *blk = createworkMlol232(label); 
    return blk;
}
...
{
    extern HSim__s6* createWork_eaadd6bit_struct(const char*);
    HSim__s6 *blk = createWork_eaadd6bit_struct(label);
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","lol232", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("eaadd6bit/struct", (HSimInstFactoryPtr)IF2, true);
        (*cfgvh).addVlogModule("eaadd6bit", (HSimInstFactoryPtr)IF3, true);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMlol232(const char*);
        topvl = (HSim__s5*)createworkMlol232("lol232");
