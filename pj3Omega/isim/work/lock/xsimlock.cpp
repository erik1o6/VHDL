...
va_list vap)
{
    extern HSim__s6 * createworkMlock(const char*);
    HSim__s6 *blk = createworkMlock(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","lock", (HSimInstFactoryPtr)IF0);
        ...
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMlock(const char*);
        topvl = (HSim__s5*)createworkMlock("lock");
