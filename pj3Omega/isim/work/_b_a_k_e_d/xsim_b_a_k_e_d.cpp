...
va_list vap)
{
    extern HSim__s6 * createworkM_b_a_k_e_d(const char*);
    HSim__s6 *blk = createworkM_b_a_k_e_d(label); 
    return blk;
}
...
{
    extern HSim__s6* createWork_ea4bitfrequencycounter_struct(const char*);
    HSim__s6 *blk = createWork_ea4bitfrequencycounter_struct(label);
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","BAKED", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("ea4bitfrequencycounter/struct", (HSimInstFactoryPtr)IF2, true);
        (*cfgvh).addVlogModule("ea4bitfrequencycounter", (HSimInstFactoryPtr)IF3, true);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkM_b_a_k_e_d(const char*);
        topvl = (HSim__s5*)createworkM_b_a_k_e_d("BAKED");
