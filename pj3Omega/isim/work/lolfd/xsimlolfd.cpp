...
va_list vap)
{
    extern HSim__s6 * createworkMlolfd(const char*);
    HSim__s6 *blk = createworkMlolfd(label); 
    return blk;
}
...
{
    extern HSim__s6* createWork_ea6bitmux_struct(const char*);
    HSim__s6 *blk = createWork_ea6bitmux_struct(label);
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","lolfd", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("ea6bitmux/struct", (HSimInstFactoryPtr)IF2, true);
        (*cfgvh).addVlogModule("ea6bitmux", (HSimInstFactoryPtr)IF3, true);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMlolfd(const char*);
        topvl = (HSim__s5*)createworkMlolfd("lolfd");
