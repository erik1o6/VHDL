...
va_list vap)
{
    extern HSim__s6 * createworkMloll23(const char*);
    HSim__s6 *blk = createworkMloll23(label); 
    return blk;
}
...
{
    extern HSim__s6* createWork_ea6flip_struct(const char*);
    HSim__s6 *blk = createWork_ea6flip_struct(label);
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","loll23", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("ea6flip/struct", (HSimInstFactoryPtr)IF2, true);
        (*cfgvh).addVlogModule("ea6flip", (HSimInstFactoryPtr)IF3, true);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMloll23(const char*);
        topvl = (HSim__s5*)createworkMloll23("loll23");
