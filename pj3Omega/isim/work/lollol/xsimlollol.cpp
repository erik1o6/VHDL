...
va_list vap)
{
    extern HSim__s6 * createworkMlollol(const char*);
    HSim__s6 *blk = createworkMlollol(label); 
    return blk;
}
...
{
    extern HSim__s6* createWork_ufm6bit_struct(const char*);
    HSim__s6 *blk = createWork_ufm6bit_struct(label);
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","lollol", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("ufm6bit/struct", (HSimInstFactoryPtr)IF2, true);
        (*cfgvh).addVlogModule("ufm6bit", (HSimInstFactoryPtr)IF3, true);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMlollol(const char*);
        topvl = (HSim__s5*)createworkMlollol("lollol");
